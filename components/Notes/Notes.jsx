...
const Notes = () => {
  const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('Manage Notes'));
    });
  ...
  const [pagedNotes, setPagedNotes] = useState(filterdNotesList);
  ...
  useEffect(() => {
    searchNotes();
  }, [selectedTab, searchNote, notesList]);
  ...
  const searchNotes = (isResetPage = true) => {
    if (isResetPage) {
        pager.currentPage = 1;
      }
    let res;
    if (selectedTab !== "fav") {
        if (selectedTab !== "all" || selectedTab === "delete") {
            res = notesList.filter((d) => d.tag === selectedTab);
        } else {
            res = notesList;
        }
      } else {
        res = notesList.filter((d) => d.isFav);
      }
      setFilterdNotesList([
      ...res.filter((d) => d.title?.toLowerCase().includes(searchNote)),
    ]);
    getPager(res.filter((d) => d.title?.toLowerCase().includes(searchNote)));
    console.log("Res", res);
    console.log("Filter", filterdNotesList)
  };
  ...
  const getPager = (res) => {
    setTimeout(() => {
      if (res.length) {
        ...
        setPagedNotes(res.slice(pager.startIndex, pager.endIndex + 1));
      } else {
        setPagedNotes([]);
        pager.startIndex = -1;
        pager.endIndex = -1;
...
//     }
//     setFilterdNotesList([
//         ...notesList.filter((d) => d.title?.toLowerCase().includes(searchNote)),
//       ]);
//   };
              ...
              <div className="space-y-1">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">All Notes</div>
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="ml-3">Personal</div>
                </button>
                <button
                  type="button"
                      ...
                      <div className="min-h-[142px]">
                        <div className="flex justify-between">
                          <div className="flex w-max items-center">
                            ...
                            <div className="ml-2">
                              <div className="font-semibold text-sm">{note.user}</div>
                        ...
                        <div>
                          <h4 className="mt-4 font-semibold text-sm">{note.title}</h4>
