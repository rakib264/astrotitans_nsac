...
const BasicDatatable = () => {
    ...
    const customFooter = 'background-color: #1a2941';

    useEffect(() => {
        // Apply the custom class to the table footer element when the component mounts
        const tableFooter = document.querySelector('.mantine-Table-root tfoot');
        if (tableFooter) {
          tableFooter.classList.add('customFooter');
        }
    
        // Clean up the class when the component unmounts
        return () => {
          if (tableFooter) {
            tableFooter.classList.remove('customFooter');
          }
        };
      }, []);
