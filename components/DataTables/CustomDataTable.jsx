...
const CustomDataTable = () => {
  ...
  const [hideCols, setHideCols] = useState(["age", "dob", "isActive"]);
...
//   },
  ...
  function excludeProperties(arr, propertiesToExclude) {
    return arr.map((item) => {
      const newObj = { ...item };
      propertiesToExclude.forEach((prop) => {
        if(prop === 'address.street'){
            const key = prop.slice(0,7);
            // console.log("Key:", key)
            delete newObj[key]
        } else{
            delete newObj[prop]
  ...
  const filteredData = excludeProperties(recordsData, hideCols);
...
//   console.log("FilteredData:", filteredData)
  ...
  function handleDownloadExcel() {
    const formattedData = filteredData?.map((item) => ({
        ...
        StartDate: item?.dob || 'Null',
        PhoneNo: item?.phone || 'Null',
        Age: item?.age || 'Null',
        Company: item?.company || 'Null',
        Address: item?.address !== undefined ? `Street: ${item?.address?.street}, 
        City: ${item?.address?.city}, Zip Code: ${item?.address?.zipcode},
        Geo: lat - ${item?.address?.geo?.lat}, long - ${item?.address?.geo?.lng}` : 'Null',
  ...
  return (
    <div>
      <div className="panel my-6">
        <h5 className="text-lg flex items-center gap-2 font-semibold dark:text-white-light mb-3">
          Column Chooser - Export - Table
